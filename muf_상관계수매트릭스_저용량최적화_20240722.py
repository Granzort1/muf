...
def optimize_png(filename):
    try:
        ...
        subprocess.run(['optipng', '-o9', unicode_filename], check=True)
    except subprocess.CalledProcessError as e:
        print(f"OptiPNG 실행 중 오류 발생: {e}")
    except FileNotFoundError:
        print("OptiPNG를 찾을 수 없습니다. PATH 환경 변수를 확인해주세요.")
...
def replay(df, filename):
    ...
    # 그래프 저장 후
    plt.savefig(f"C:/muf/result/{filename}_cor_matrix.png", dpi=350)
    optimize_png(f"C:/muf/result/{filename}_cor_matrix.png")
    plt.close()
