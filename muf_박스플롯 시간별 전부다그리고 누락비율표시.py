...
def calculate_missing_percentage(ob1_min):
    # Calculate the total hours in the dataset
    total_hours = (ob1_min['datetime'].max() - ob1_min['datetime'].min()).total_seconds() // 3600

    # Create new columns for date and hour
    ob1_min['hour'] = ob1_min['datetime'].dt.hour
    ob1_min['date'] = ob1_min['datetime'].dt.date

    # Count the number of datapoints for each hour
    counts_per_hour = ob1_min.groupby(['date', 'hour']).size()

    # Calculate the number of missing datapoints in each hour
    missing_counts = (60 - counts_per_hour[counts_per_hour < 60]).sum()

    # Calculate and return the missing percentage
...
def graph(ob1_min, title_name):
    ...
    for model_one in models_one:
        ...
        for start, end in missing_periods:
            if start == end:
                plt.scatter(start, ob1_min[f"{model_one}"].min(), color='red',
                            s=1)  # draw a dot for single missing hour
            else:
                plt.hlines(y=ob1_min[f"{model_one}"].min(), xmin=start, xmax=end,
        ...
        missing_percentage = calculate_missing_percentage(ob1_min)
        ...
        #
        # # 평균값 계산
        # ob1_min["tmfc_h"] = ob1_min["tmfc_h"] % 24
