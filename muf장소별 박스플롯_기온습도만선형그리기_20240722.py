...
for pollutant in pollutants:
    ...
    ax.set_yscale("linear")  # Changed from "linear" to "linear"
    ...
    min_value = min(series.min() for series in data_all_sites.values() if not series.empty)

    ax.set_ylim([min_value, ax.get_ylim()[1]])

    # Set colors for x-tick labels
