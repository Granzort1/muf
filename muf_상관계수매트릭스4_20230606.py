...
def replay(df):
    ...
    def scatterplot_with_regression(x, y):
        ...
        sns.regplot(x=downsampled_df[x.name], y=downsampled_df[y.name], scatter=False, ci='None')
        plt.xlim(x.quantile(0.01), x.quantile(0.99))
        plt.ylim(y.quantile(0.01), y.quantile(0.99))

    grid.map_lower(scatterplot_with_regression)
