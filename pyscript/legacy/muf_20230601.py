...
def graph(ob1_min, title_name):
    ...
    #     plt.tight_layout()
    #     plt.savefig(f"C:/muf/x_temperature/{model.replace(' ~ ', '_').replace('np.log(', '').replace(')', '')}_S1{title_name}.png", dpi=500)
    #     plt.close()
...
# Time series plot
        ...
        plt.figure(figsize=(14, 7))
        ob1_min_sorted = np.sort(ob1_min[f"{model_one}"])
        norm_data = norm.ppf(np.arange(1, len(ob1_min) + 1) / (len(ob1_min) + 1))
        plt.plot(norm_data, ob1_min_sorted, "o", color='black', alpha=0.5)
        plt.plot([min(norm_data), max(norm_data)], [min(ob1_min_sorted), max(ob1_min_sorted)], color='blue')
        ...
        plt.tight_layout()
        plt.savefig(f"{model_one}_QQplot({title_name}).png", dpi=370)
        plt.close()
