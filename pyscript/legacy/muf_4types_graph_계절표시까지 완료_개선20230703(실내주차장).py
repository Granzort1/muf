...
def graph(ob1_min, title_name):
    ...
    for model_one in models_one:
        ...
        plt.plot(ob1_min['datetime'], ob1_min[f"{model_one}"],'o', markersize=1, label=f"{model_one}", color='black')
        #plt.yscale("symlog")
        ...
        plt.tight_layout()
        plt.savefig(f"C:\\muf\\graph\\{model_one}_time({title_name})(logscale).png", dpi=370)
        plt.close()
