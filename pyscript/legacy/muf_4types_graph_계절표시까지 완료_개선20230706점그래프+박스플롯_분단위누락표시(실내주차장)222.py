...
def graph(df, title_name, substances):
    ...
    for model_one in models_one:
        ...
        for substance in substances:
            substance_data = df[["datetime", substance]]

            # Filter out datasets where all measurements are 0
            if not (substance_data == 0).all().all():
                missing_data_ratios[substance] = compute_missing_data_ratio(substance_data)
...
files = {
    "가산A1타워주차장": "C:/muf/가산A1타워주차장_20230331.csv",
    "에이샵스크린골프": "C:/muf/에이샵스크린골프_20230331.csv",
    "영통역대합실": "C:/muf/영통역대합실_20230331.csv",
    "영통역지하역사": "C:/muf/영통역지하역사_20230331.csv",
    "이든어린이집": "C:/muf/이든어린이집_20230331.csv",
    "좋은이웃데이케어센터1": "C:/muf/좋은이웃데이케어센터1_20230331.csv",
    "좋은이웃데이케어센터2": "C:/muf/좋은이웃데이케어센터2_20230331.csv",
    "좋은이웃데이케어센터3": "C:/muf/좋은이웃데이케어센터3_20230331.csv",
    "좋은이웃데이케어센터4": "C:/muf/좋은이웃데이케어센터4_20230331.csv",
    "하이씨앤씨학원": "C:/muf/하이씨앤씨학원_20230331.csv"
}

datasets = {place: pd.read_csv(path, encoding='utf-8') for place, path in files.items()}
substances = ["pm10", "pm25", "pm1", "humi", "temp", "hcho", "co", "no2", "rn", "voc"]
missing_data_ratios = {}


# List of pollutants
pollutants = ["pm10", "pm25", "pm1", "humi", "temp", "hcho", "co", "no2", "rn", "voc", "co2", "tab"]

results = {}
overall_results = {}
for name, df in datasets.items():
    missing_data_ratios[name] = graph(df, name, substances)

    # Calculate overall mean and std_dev for each substance if there are any values
    for column in df.columns:
        if results[column]:  # Check if there are any values
            means = [x[0] for x in results[column]]
            std_devs = [x[1] for x in results[column]]
            overall_mean = np.mean(means)
            overall_std_dev = np.std(std_devs)
            overall_results[column].append((overall_mean, overall_std_dev))
