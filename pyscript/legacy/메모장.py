...
def graph(ob1_min, title_name):
    ...
    for model_one in models_one:
        ...
        data_numpy = data_its.to_numpy()

        # 0 이하의 값을 제외
        data_positive = data_numpy[data_numpy > 0]
        ...
        # x축과 y축의 눈금 설정
        xticks = np.linspace(xmin, xmax, 5)  # 5개의 눈금으로 설정 (적절한 개수로 조정 가능)
        yticks = np.linspace(ymin, ymax, 5)  # 5개의 눈금으로 설정 (적절한 개수로 조정 가능)
        ax.set_xticks(xticks)
        ax.set_yticks(yticks)

        # x축과 y축의 눈금 레이블 설정
        xticklabels = [f"{tick:.2f}" for tick in xticks]  # 소수점 둘째 자리까지 표시
        yticklabels = [f"{tick:.2f}" for tick in yticks]  # 소수점 둘째 자리까지 표시
        ...
        # x축과 y축의 눈금 레이블 설정
        xticklabels = [f"{tick:.2e}" for tick in xticks]  # 지수 표기법으로 표시
        yticklabels = [f"{tick:.2e}" for tick in yticks]  # 지수 표기법으로 표시
        ax.set_xticklabels(xticklabels)
        ax.set_yticklabels(yticklabels)
