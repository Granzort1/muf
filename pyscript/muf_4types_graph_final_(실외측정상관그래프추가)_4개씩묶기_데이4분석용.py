...
def graph(ob_min, title_name, outdoor_data, column_mapping):
    ...
    ob_min['datetime'] = pd.to_datetime(ob_min['tmfc_d'].astype(str) + ob_min['tmfc_h'].astype(str).str.zfill(2),
                                        format='%Y%m%d%H')
    print(ob_min['datetime'].min(), ob_min['datetime'].max())
    unique_dates = ob_min['datetime'].dt.date.unique()
    print(unique_dates)
    ...
    for model_one in models_one:
        ...
        if scale_y == "log":
            ...
            ax1.plot(ob_min['datetime'], data_its, 'o', markersize=1, label=f"{model_name_4th}", color='black',
                     markeredgewidth=0)
            ax1.axhline(y=cut_conc, color=col, linestyle='--')
