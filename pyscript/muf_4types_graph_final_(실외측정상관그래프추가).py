...
def graph(ob_min, title_name, outdoor_data, column_mapping):
    ...
    for model_one in models_one:
        ...
        missing_data_ratios[model_one] = compute_missing_data_ratio(ob_min)

        data_its = ob_min[f"{model_one}"]

        # Time Series Plot
        plt.figure(figsize=(14, 7))
        if scale_y == "log":
            data_its_positive = data_its[data_its > 0]  # Exclude 0 and negative values
            datetime_positive = ob_min['datetime'][data_its > 0]  # Filter datetime correspondingly
            ...
            plt.plot(ob_min['datetime'], data_its, 'o', markersize=1, label=f"{model_name_4th}", color='black',
                     markeredgewidth=0)
            plt.axhline(y=cut_conc, color=col, linestyle='--')
        ...
        all_dates = pd.date_range(start=ob_min['datetime'].min(), end=ob_min['datetime'].max(), freq='H')
        incomplete_hours = ob_min['datetime'].value_counts().loc[lambda x: x < 60].index
        missing_dates = pd.Index(all_dates).difference(ob_min['datetime']).union(incomplete_hours)
        ...
        for year in ob_min['datetime'].dt.year.unique():
            for month in [2, 5, 8, 11]:
                if (ob_min['datetime'] >= datetime(year, month, 1)).any() and (
                        ob_min['datetime'] < datetime(year, month + 1, 1)).any():
                    plt.axvline(x=datetime(year, month + 1, 1), color='blue', linestyle='--')

        plt.tight_layout()
        plt.savefig(f"C:\\muf\\graph\\{title_name}\\{model_one}_timeseries({title_name})({scale_name}).png", dpi=370)
        plt.close()
        ...
        if scale_y == "log":
            ...
            plt.tight_layout()
            plt.savefig(f"C:\\muf\\graph\\{title_name}\\{model_one}_QQplot({title_name})(log10scale).png", dpi=370)
        else:
            ...
            plt.tight_layout()
            plt.savefig(f"C:\\muf\\graph\\{title_name}\\{model_one}_QQplot({title_name}).png", dpi=370)
        plt.close()
        ...
        plt.tight_layout()
        plt.savefig(f"C:\\muf\\graph\\{title_name}\\{model_one}_histogram({title_name})({scale_name}).png", dpi=370)
        plt.close()
        ...
        plt.tight_layout()
        plt.savefig(f"C:\\muf\\graph\\{title_name}\\{model_one}_boxplot({title_name})({scale_name}).png", dpi=370)
        plt.close()
        ...
        plt.figure(figsize=(14, 7))
        means = data_its.groupby(ob_min["tmfc_h"]).mean()

        if scale_y == "log":
            data_its_positive = data_its[data_its > 0]  # Exclude 0 and negative values
            tmfc_h_positive = ob_min["tmfc_h"][data_its > 0]  # Filter tmfc_h correspondingly
            ...
            box_plot = sns.boxplot(x=ob_min["tmfc_h"], y=data_its, color='white', order=hours)
            plt.scatter(x=means.index, y=means.values, color='red', zorder=10)
            plt.axhline(y=cut_conc, color=col, linestyle='--')
            plt.ylabel(f"{model_name_label}")
        ...
        plt.tight_layout()
        plt.savefig(f"C:\\muf\\graph\\{title_name}\\{model_one}_hourly_boxplot({title_name})({scale_name}).png",
                    dpi=370)
        plt.close()

        if model_one in ['pm10', 'pm25', 'co', 'no2']:
            # 실내 데이터를 시간 단위로 그룹화하고 평균 계산
            indoor_data_hourly = ob_min.groupby(['tmfc_d', 'tmfc_h'])[model_one].mean().reset_index()
            ...
            plt.legend()
            plt.tight_layout()
            plt.savefig(f"C:\\muf\\graph\\{title_name}\\{model_one}_in_vs_out({title_name}).png", dpi=370)
            plt.close()
...
indoor_locations = ["가산A1타워주차장", "에이샵스크린골프", "영통역대합실", "영통역지하역사", "이든어린이집", "좋은이웃데이케어센터1", "좋은이웃데이케어센터2", "좋은이웃데이케어센터3", "좋은이웃데이케어센터4", "하이씨앤씨학원"]
